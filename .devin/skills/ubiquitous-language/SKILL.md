...
description: Scan the codebase + docs and maintain .devin/ubiquitous_language.md — the canonical AIOS glossary
...
(`.devin/ubiquitous_language.md`). This fills a real gap: AIOS has rich domain
vocabulary scattered across `docs/manifesto.md`, `docs/roadmap.md`,
`docs/safety.md`, `services/aios_core/intent.py (inline schema)`, and code docstrings, with
...
(or user) then applies the edits to `.devin/ubiquitous_language.md`. This keeps
...
3. `docs/safety.md` and `docs/safety.md` — safety-spine
   vocabulary (spine, layers, Sherpa, fail-closed, etc.).
4. `docs/operations.md` and `docs/operations.md` — operational/runtime terms.
5. `services/aios_core/intent.py (inline schema)` — intent-hierarchy terms (manifesto, charter,
...
rather than silently picking one. Per `docs/README.md`, `docs/operations.md` and
...
1. **New terms** to add to `.devin/ubiquitous_language.md` (with the fields
   above).
...
Do not edit `.devin/ubiquitous_language.md` yourself (read-only subagent). The
